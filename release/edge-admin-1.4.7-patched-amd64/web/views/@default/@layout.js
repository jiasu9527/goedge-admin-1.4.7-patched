Tea.context(function () {
	...
	this.buildFallbackTeaModules = function () {
		let modules = [
			{
				code: "dashboard",
				name: "看板",
			...
			},
			{
				...
				icon: "clone outsize",
				subItems: [
					{name: "访问日志", url: "/servers/logs", code: "log"},
					{name: "SSL证书", url: "/servers/certs", code: "cert"},
					{name: "分组", url: "/servers/groups", code: "group"},
					{name: "-", url: "", code: ""},
					{name: "缓存策略", url: "/servers/components/cache", code: "cache"},
					{name: "WAF策略", url: "/servers/components/waf", code: "waf"},
					{name: "统计指标", url: "/servers/metrics", code: "metric"},
				],
			},
			{
				code: "clusters",
				...
				icon: "cloud",
				subItems: [
					{name: "节点集群", url: "/clusters", code: "cluster"},
					{name: "节点日志", url: "/clusters/logs", code: "log"},
					{name: "区域", url: "/clusters/regions", code: "region"},
					{name: "SSH授权", url: "/clusters/grants", code: "grant"},
				],
			},
			{
				code: "dns",
				...
				icon: "globe",
				subItems: [
					{name: "DNS集群", url: "/dns", code: "cluster"},
					{name: "服务商", url: "/dns/providers", code: "provider"},
					{name: "问题排查", url: "/dns/issues", code: "issue"},
				],
			},
		]
		...
		if (this.teaIsPlus) {
			modules.push({
				...
				icon: "cubes",
				subItems: [
					{name: "域名列表", url: "/ns/domains", code: "domain"},
					{name: "DNS集群", url: "/ns/clusters", code: "cluster"},
					{name: "自定义线路", url: "/ns/routes", code: "route"},
					{name: "套餐管理", url: "/ns/plans", code: "plan"},
					{name: "用户套餐", url: "/ns/userPlans", code: "userPlan"},
				],
			})
		}
		...
		modules.push(
			{
				...
				icon: "users",
				subItems: [
					{name: "用户列表", url: "/users", code: "users"},
				],
			},
			{
				code: "admins",
		...
		if (this.teaShowFinance !== false) {
			modules.push({
				code: "finance",
				name: "财务",
				icon: "yen sign",
				subItems: [
					{name: "计费设置", url: "/finance/fee", code: "fee"},
					{name: "账户", url: "/finance/accounts", code: "account"},
					{name: "订单", url: "/finance/orders", code: "order"},
					{name: "账单", url: "/finance/bills", code: "bill"},
					{name: "流量包", url: "/finance/packages", code: "package"},
					{name: "收入统计", url: "/finance/income", code: "income"},
					{name: "流水日志", url: "/finance/logs", code: "log"},
				],
			})
		}
		...
		if (this.teaIsPlus) {
			modules.push(
				...
				},
				{
					...
					icon: "ticket",
					subItems: [
						{name: "进行中", url: "/tickets?status=none", code: "none"},
						{name: "已解决", url: "/tickets?status=solved", code: "solved"},
						{name: "已关闭", url: "/tickets?status=closed", code: "closed"},
					],
				},
			)
		}
		...
		modules.push(
			...
			},
			{
				...
				icon: "setting",
				subItems: [
					{name: "基础设置", url: "/settings", code: "basic"},
					{name: "高级设置", url: "/settings/advanced", code: "advanced"},
					{name: "商业授权", url: "/settings/authority", code: "authority"},
				],
			},
		)
	...
	this.ensureTeaModules = function () {
		if (Array.isArray(this.teaModules) && this.teaModules.length > 0) {
			return
		}

		this.teaModules = this.buildFallbackTeaModules()

		if (window.TEA != null && window.TEA.ACTION != null && window.TEA.ACTION.data != null) {
			window.TEA.ACTION.data.teaModules = this.teaModules
		}
	}

	// Some patched builds return an empty teaModules array; fill it only in that case.
	this.ensureTeaModules()
	...
	this.iconMap = {
		...
		'setting': 'pi-cog',
		// EdgeUser
		'clone outline': 'pi-clone',
		'magnet': 'pi-magnet',
		'puzzle piece': 'pi-puzzle',
...
Vue.component("file-upload", {
	...
	<div class="b-file-upload-container">
		<input type="file" :name="name" :accept="accept" :multiple="multiple" @change="onFileChange" />
		<button class="b-file-upload" v-if="this.files.length===0">
			<span class="p-button-icon p-button-icon-left pi pi-plus" data-pc-section="icon"></span>
			<span>选择文件</span>
		</button>
		<span v-else-if="this.files.length==1">
			已选择：{{this.files[0].name}}
		</span>
		<span v-else>
			已选择：{{this.files[0].name}}等，{{this.files.length}}个文件
		</span>
	</div>
	`,
});
